...
<style>
  ...
  .exit-box { position:relative; z-index:1; background:#0A0A0A; border:2px solid #FFD400; max-width:480px; width:100%; padding:2.5rem 2rem 2rem; text-align:center; animation:exitPop .35s cubic-bezier(.34,1.56,.64,1); }
  @keyframes exitPop { from{transform:scale(.85);opacity:0} to{transform:scale(1);opacity:1} }
  .exit-close { position:absolute; top:.75rem; right:1rem; background:none; border:none; color:#555; font-size:1.75rem; line-height:1; cursor:pointer; transition:color .2s; }
  .exit-close:hover { color:#FFD400; }
  .exit-badge { display:inline-block; background:#FFD400; color:#000; font-size:10px; font-weight:700; letter-spacing:3px; text-transform:uppercase; padding:4px 14px; margin-bottom:1.25rem; }
  .exit-top { margin-bottom:1rem; }
  .exit-logo { height:48px; width:auto; filter:brightness(0) invert(1); opacity:.85; }
  .exit-title { font-family:'Bebas Neue',sans-serif; font-size:clamp(2rem,7vw,2.8rem); line-height:1; letter-spacing:1px; color:#fff; margin-bottom:1rem; }
  .exit-title span { color:#FFD400; }
  .exit-sub { color:#888; font-size:14px; line-height:1.6; margin-bottom:1.25rem; }
  .exit-discount { font-family:'Bebas Neue',sans-serif; font-size:4rem; color:#FFD400; line-height:1; letter-spacing:2px; margin-bottom:1.25rem; }
  .exit-coupon-wrap { background:#111; border:1.5px dashed #FFD400; padding:1rem 1.25rem; margin-bottom:1.5rem; }
  .exit-coupon-label { display:block; font-size:10px; font-weight:700; letter-spacing:2px; color:#555; margin-bottom:.5rem; }
  .exit-coupon-row { display:flex; align-items:center; justify-content:center; gap:.75rem; }
  .exit-coupon-code { font-family:'Bebas Neue',sans-serif; font-size:1.8rem; letter-spacing:3px; color:#FFD400; }
  .exit-copy-btn { background:#FFD400; color:#000; border:none; font-family:'DM Sans',sans-serif; font-weight:700; font-size:11px; letter-spacing:1px; padding:6px 14px; cursor:pointer; transition:opacity .2s; white-space:nowrap; }
  .exit-copy-btn:hover { opacity:.85; }
  .exit-copy-btn.copied { background:#38A169; color:#fff; }
  .exit-cta { display:block; background:#FFD400; color:#000; font-family:'DM Sans',sans-serif; font-weight:700; font-size:14px; letter-spacing:1.5px; text-transform:uppercase; padding:14px; text-decoration:none; margin-bottom:.875rem; transition:opacity .2s; }
  .exit-cta:hover { opacity:.9; }
  .exit-skip { background:none; border:none; color:#444; font-family:'DM Sans',sans-serif; font-size:12px; cursor:pointer; text-decoration:underline; transition:color .2s; }
  .exit-skip:hover { color:#888; }
  @media(max-width:480px) { .exit-box{padding:2rem 1.25rem 1.5rem} .exit-discount{font-size:3rem} .exit-coupon-code{font-size:1.5rem} }
</style>
